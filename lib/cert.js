...
function Cert(opts) {
    ...
    if (typeof(this.opts.na) === "string") {
        this.opts.na = moment(this.opts.na).toISOString();
    }
...
Cert.prototype.crunch = function (cb) {
    ...
    var prom = makeKeyPair(self.opts.b).then(function (keys) {
        ...
        return makeCert(keys, self.opts.name, self.opts.org, self.opts.alt).then(function (cert) {
            self.cert = cert;
            return cert;
        });
...
function makeCert(keys, name, org, alts) {
    return new Promise(function (res) {
