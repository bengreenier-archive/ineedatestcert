...
function Cert(opts) {
    ...
    if (typeof(this.opts.alt) === "string") {
        this.opts.alt = [this.opts.alt];
    }
    if (typeof(this.opts.na) !== "string") {
        throw new Error("na should be a datetime string (can be empty)");
    }
    
    this.opts.na = moment(this.opts.na);
    ...
    if (!this.opts.alt || typeof(this.opts.alt.length) !== "number") {
        throw new Error("alt should be an array of dns names");
    }
...
Cert.prototype.crunch = function (cb) {
    ...
    var prom = makeKeyPair(self.opts.b).then(function (keys) {
        ...
        return makeCert(keys, self.opts.name, self.opts.org, self.opts.alt, self.opts.na).then(function (cert) {
            self.cert = cert;
            return cert;
        });
...
function makeCert(keys, name, org, alts, na) {
    return new Promise(function (res) {
        ...
        cert.validity.notAfter = na.toDate();
